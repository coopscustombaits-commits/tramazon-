 ...
 *
 * NOTE: this file is a copy of `src/lib/moderation.ts`. The two directories
 * are separate npm packages with separate builds, so the app cannot import
 * across the boundary. A unit test compares them byte for byte, so they
 * cannot drift — change one and the test tells you to change the other.
 */
